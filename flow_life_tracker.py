...
# --- SETUP ---
log_name = "flow_life_tracker"
logger = setup_logger(name=log_name, log_filename="flow_life.log")
...
# ID 3 is dedicated to Flow Life Tracker
FLOW_LIFE_WORKER_ID = 3

def get_last_processed_date():
    """Fetch the last successfully processed date from DB."""
    try:
        resp = supabase.table("worker_status").select("last_run").eq("id", FLOW_LIFE_WORKER_ID).execute()
...
def process_trade_state(trade, high, low, close, oi, current_status, exp_date_str, current_date=None):
    ...
    # Default profit target if not set
    target = float(trade.get('profit_target') or (entry * 1.20))
    # Default stop_oi if not set (safe fallback)
    stop_oi_level = int(trade.get('stop_oi_level') or 0)
    ...
    # Logic for manual trades might differ slightly (e.g. maybe no stop_oi for manual?)
    # Keeping logic same as requested.
    
    if current_status == "OPEN" and check_date > entry_date:
        # For manual trades, 'entry_oi' might be null if not fetched.
        # We can assume if stop_oi_level is 0, we might need to set it?
        # Leaving logic as is: if stop_oi_level is set, we check it.
        pass
    ...
    if current_status == "SCALED":
        ...
        if new_highest >= target:
            ...
            close = 0.0
        elif check_date > entry_date and stop_oi_level > 0:
            if oi < stop_oi_level:
                new_status = "STOP_OI"
...
def process_single_day(processing_date):
    ...
    # Fetch from FLOW_LIFE schema
    response = supabase.schema("flow_life").table("trades").select("*").in_("status", ["OPEN", "SCALED"]).execute()
        ...
        try:
            ...
            if not market_data:
                ...
                if processing_date >= exp_dt:
                    logger.warning(f"      🧟 Zombie Trade Detected: {trade['ticker']} (Exp: {exp_dt})")
                    market_data = {
            ...
            # 3. UPDATE DB (Flow Life Schema)
            supabase.schema("flow_life").table("trades").update(payload).eq("id", trade['id']).execute()
            
            supabase.schema("flow_life").table("trade_performance").upsert({
                "trade_id": trade['id'],
            ...
            }, on_conflict="trade_id, date").execute()
...
def run_smart_catchup():
    logger.info("🚀 Starting Flow-Life Tracker...")
